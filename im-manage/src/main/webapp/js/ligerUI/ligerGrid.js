        ...
        */
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                //alert(p.isChecked(item));
                //alert(item.id);
                if (!frozen && g.enabledCheckbox() && p.isChecked && p.isChecked(item))
                {
                	var rowParent = g.getParent(item);
                	var isSelect = g.isSelected(rowParent);
                	//alert(g.isSelected(rowParent));
                	if(rowParent !=  null && !isSelect) {
                		g['select'](rowParent);
                	}
                	
                	//var row = $(rowParent.row);
                	//var domId = g._getRowDomId(rowParent,false);
                	//var td = document.getElementById("maingrid|2|r1001"); 
            		//alert(td);
            		//var clazz = td.getAttribute("class");
            		//td.className = clazz + " l-selected";
            		//alert(td.className);
                	
                	//alert(row.hasClass("l-grid-row-cell-inner"));
                	
                	//var trs = $('#'+domId);
                	//alert(trs.attr("class"));
                	//trs.addClass(" l-selected");
                	
                	//alert(rowParent.toString());
                	//var rowdata = this.getRow(rowParent);
                	//alert(rowdata['__id']);
                	//g.select(g._getRowDomId(rowdata, frozen));
                	
                	//var row = $(rowParent.row);
                	//alert(row);
                	//row.addClass("l-selected");
                	//rowParent.push(' l-selected');
        ...
        _getTreeCellHtml: function (oldContent, rowdata)
        {
            ...
            //bug 第二次打开dialog时,里面的grid报错:p.tree.isParent Is not a function    //来自csdn  ligerUI bug 一些解决办法
            p.tree.isParent = p.tree.isParent || function (rowData)
                {
                    var exist = p.tree.childrenName in rowData;
                    return exist;
        ...
        _onClick: function (e)
        {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly)
                {
                    ...
                    var flag = false;
                    var parentChildren = g._getParentChildren(src.data);
                    for (var i = 0, l = parentChildren.length; i < l; i++)
                    {
                    	flag = g.isSelected(parentChildren[i]);
                    	if(flag) {
                    		break;
                    	}
                    	//alert(parentChildren[i].id);
                    	//alert(g.isSelected(parentChildren[i]));
                        //var rowdata = this.getRow(parentChildren[i]);
                        //alert(rowdata);
                    }
                    //alert(met);
                    //alert(flag);
                    if(!flag && met == 'unselect') {
                    	var rowParent = g.getParent(src.data);
                    	if(rowParent != null) {
        	            	g[met](rowParent);
        	            }
                    } 
