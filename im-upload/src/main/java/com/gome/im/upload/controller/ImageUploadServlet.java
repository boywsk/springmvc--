...
import com.gome.im.upload.service.impl.ImageUploadServiceImpl;
...
public class ImageUploadServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 log.info("Receive Message: appId:"+appId+", uid:"+uid+", time:"+time+", key:"+key+", traceId:"+traceId+", port:"+request.getLocalPort());
		 try{
			 ...
			 if(key.equals(autMd5)){			//判断Md5加密
	             ...
	             //if(nowTime-time <= 180000){  	//小于等于3分钟，3*60*1000
					 ...
					 if(suffix_F != null) {		//判断文件格式
						 ...
						 ImageUploadServiceImpl uploadService = new ImageUploadServiceImpl();
						 log.info("traceId:"+traceId+"; sourceFileName:"+fileName+"; fileSize:"+input.available());
						 rm = uploadService.uploadImg(input, suffix_F, true,uid,traceId);						
						}else{
							rm.setCode(1);
