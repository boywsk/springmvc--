...
import com.gome.im.upload.service.impl.VedioUploadServiceImpl;
import com.gome.im.upload.utils.Md5Util;

@WebServlet(name="VedioUploadServlet",urlPatterns="/VedioUploadServlet")
@MultipartConfig
public class VedioUploadServlet extends HttpServlet {
	private static Logger log = LoggerFactory.getLogger(VedioUploadServlet.class);
	 /**
	 */
			 ...
			 if(key.equals(autMd5)){			//判断Md5加密
	             ...
	             //if(nowTime-time <= 180000){  	//小于等于3分钟，3*60*1000
					 ...
					 try{
				         ...
				         File f = new File("/data/im-upload",fileName);
//				         File f = new File("C:\\ffmpeg\\input\\",fileName);
...
//					 if(suffix_F != null) {		//判断文件格式
						 ...
						 VedioUploadServiceImpl uploadService = new VedioUploadServiceImpl();
						 log.info("traceId:"+traceId+"; sourceFileName:"+fileName+"; fileSize:"+input.available());
						 rm = uploadService.uploadVedio(input, fileName, uid,traceId);
//						}else{
//							rm.setCode(1);
//							rm.setMessage("File format Exception!");
//							log.error("File format Exception!");
//						}
	             /*}else{
	            	 rm.setCode(1);
