...
public class UploadFileController {
	...
	public ResultModel<Object> uploadImage(@RequestParam(value = "token", required = true) String token,
		...
		if (file==null) {
			log.error("uploadImage file is null");
		}else{
			log.info("uploadImage fileName:[{}]", file.getOriginalFilename());
		}
		InputStream input = null;
		Constant.FILE_SUFFIX fileSuffix = null;//文件格式 后缀扩展名
		try {
	...
	public Constant.FILE_SUFFIX getSuffix(String suffix) {
		Constant.FILE_SUFFIX fileSuffix = null;
		Constant.FILE_SUFFIX[] fileSuffixs = Constant.FILE_SUFFIX.values(); //允许上传的所有文件格式
		for (Constant.FILE_SUFFIX suf : fileSuffixs) {
