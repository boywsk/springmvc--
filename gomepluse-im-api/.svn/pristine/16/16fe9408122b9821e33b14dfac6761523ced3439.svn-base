...
//		parm2.put("friendUserId", userId);
		...
		friend1.setStatus(Constant.FRIEND_STATUS.E_FRIEND_STATUS_NOT.value);
		long nowTime=System.currentTimeMillis();
		friend1.setUpdateTime(nowTime);
		...
		friend2.setUpdateTime(nowTime);
		
		int deleteFriendRowCount = friendMapper.updateFriendStatus(friend1);
		logger.info("删除好友关系,userId:{},friendUserId:{},rowCount:{}",userId,friendUserId,deleteFriendRowCount);
		int deleteFriendRowCount2 = friendMapper.updateFriendStatus(friend2);
	...
	public ResultModel<Object> listFriends(ReqFriend reqFriend,String appId,long userId) {
		...
		long lastPullTimestamp = reqFriend.getLastPullTimestamp();
		parm.put("lastPullTimestamp", lastPullTimestamp);
		try {
			...
			List<RspFriend> rspFriendList = getRspFriendList(friends,appId);
			Map<String, Object> resultMap=new HashMap<String, Object>();
			resultMap.put("arr", rspFriendList);
			resultMap.put("timestamp",lastPullTimestamp);
			
			return new ResultModel<Object>(ResultModel.RESULT_OK,"获取好友列表成功", resultMap);
		} catch (Exception e) {
