...
<script type="text/javascript">
	...
	function del(ids){
		...
		if(confirm("是否删除?") ){
			location.href = "delUser.do?ids=" + ids;
		}else{
			return;
		}
	}
...
<body>
	...
	<div>
		<table>
			<tr><td width="6%">全选<input type="checkbox" name="CheckedAll" id="CheckedAll"></td><td width="8%">id</td><td>姓名</td><td>用户名</td><td width="13%">创建时间</td><td width="18%">操作</td></tr>
			<c:forEach var="member" items="${users}" varStatus="status">
			<tr <c:if test="${status.index%2!=0 }"> style="background-color: #E5DBE2;"</c:if>>
				<td><input type="checkbox" name="chk_list" value="${member.id}"></td>
				...
				<td>
				<a href="preEditUser.do?userName=${member.userName}" class="del">修改</a>
				<!-- c:if test="${member.id != 1}" -->
					<!-- a href="preAddPermit.do?uid=${member.id}" class="del">授权</a -->
				<!-- /c:if -->
				<a href="javascript:void(0);" onclick='del(<c:out value="${member.id}"/>);' class="del">删除</a>
				</td>
			</tr>
			</c:forEach>
		</table>
