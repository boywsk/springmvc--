	...
	$.fn.preview = function(){
		...
		$(this).each(function(){
			$(this).hover(function(e){
				if(/.png$|.gif$|.jpg$|.jpeg$|.gif$|.bmp$/.test($(this).attr("href").toLowerCase())){
					$("body").append("<div id='preview'><div><img src='"+$(this).attr('href')+"' /><p>"+$(this).attr('title')+"</p></div></div>");
				}else if(/.amr$/.test($(this).attr("href").toLowerCase())) {
	                 //var r = $.getUrlParam ($(this).attr('href'),'url');
					var uri = $(this).attr('href');
					var url = uri.substring(uri.indexOf("=") + 1, uri.length);
					$("body").append("<div id='preview'><div><iframe align=\"middle\" scrolling=\"no\" width=\"210px\" height=\"25px\" src=\"../play.jsp?url="+url+"\"></iframe><p>"+$(this).attr('title')+"</p></div></div>");
				} else{
