...
public class TFSUploadFileUtil {
	...
	public static String getFileName() {
		...
		try {
			...
			log.error("error msg : tfsManager.newTfsFileName() return null");
		}
		return fileName;
	}
	 ...
	 * @return 返回文件名
	 */
	public static String uploadFile(String newTfsFileName,byte[] bytes, String suffix) {
		...
		try {
			if (bytes != null && suffix != null && !suffix.equals("")) {
				...
				if (tfsReturnFileName == null) {
					log.error("error msg : tfsManager.saveFile return null");
					return null;
				}
